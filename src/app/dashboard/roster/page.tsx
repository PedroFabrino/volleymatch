        ...
        <div className="mx-auto max-w-5xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1">
              <div className="bg-white dark:bg-gray-900 border dark:border-gray-800 rounded-2xl shadow p-6 sticky top-6 transition-colors">
                <h2 className="text-xl font-bold mb-4 border-b dark:border-gray-800 pb-2 dark:text-gray-100">Add New Player</h2>
                
                {searchParams?.error && (
                  <div className="mb-4 p-3 bg-red-100 text-red-700 rounded-lg text-sm font-semibold border border-red-200">
                    A player with this name already exists. Please use a unique name (e.g. "Guilherme S.").
                  </div>
                )}
                
                <form action={addPlayer} className="flex flex-col gap-4">
                <div className="flex flex-col gap-1.5">
                  <label htmlFor="name" className="text-sm font-semibold text-gray-700 dark:text-gray-300">Player Name</label>
                  <input 
                    id="name" 
                    name="name" 
                    type="text" 
                ...
                <div className="flex flex-col gap-1.5">
                  <label htmlFor="initial_tier" className="text-sm font-semibold text-gray-700 dark:text-gray-300">Initial Skill Tier</label>
                  <select 
                    id="initial_tier" 
                    name="initial_tier"
                    className="rounded-lg border dark:border-gray-700 p-2 text-black dark:text-white bg-white dark:bg-gray-800 focus:ring-2 focus:ring-blue-500 outline-none"
                  >
                    <option value="Beginner">Beginner (~800 MMR)</option>
                    <option value="Intermediate">Intermediate (~1000 MMR)</option>
                    <option value="Advanced">Advanced (~1200 MMR)</option>
                  </select>
                </div>

                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-semibold text-gray-700 dark:text-gray-300">Positions (Select all that apply)</label>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white dark:bg-gray-900 border dark:border-gray-800 rounded-2xl shadow p-6 transition-colors">
              <h2 className="text-xl font-bold mb-4 border-b dark:border-gray-800 pb-2 dark:text-gray-100">Current Players ({players?.length || 0})</h2>
                    ...
                    <div key={player.id} className="border dark:border-gray-700 rounded-xl p-4 flex flex-col gap-3 hover:border-blue-300 dark:hover:border-blue-500 transition">
                      <div className="flex justify-between items-start">
                        ...
                        <form action={async () => {
                          ...
                          <button type="submit" className="text-gray-400 hover:text-red-500 p-1 transition" title="Remove player">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {player.positions && player.positions.length > 0 ? (
                            player.positions.map((pos: string) => (
                              <span key={pos} className="bg-gray-100 dark:bg-gray-800 border dark:border-gray-700 text-xs px-1.5 py-0.5 rounded">{pos}</span>
                            ))
                          ) : (
                            <span className="text-gray-400 dark:text-gray-500 italic">No position set</span>
                          )}
                        </div>
                      </div>
                    </div>

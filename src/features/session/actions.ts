...
export async function startSession(formData: FormData) {
  ...
  await deactivateAllSessions(supabase, user.id)
  ...
  const presentPlayers = await getPresentPlayersWithMmr(supabase, user.id)
...
export async function endSession(sessionId: string) {
  ...
  await endSessionRecord(supabase, sessionId, user.id, summaryData)
  await resetPlayerDailyStats(supabase, user.id)
